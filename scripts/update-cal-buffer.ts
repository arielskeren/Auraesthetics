...
// Buffer time in minutes (15 minutes before each appointment)
const BUFFER_TIME_MINUTES = 15;

// Update event type buffer time in Cal.com
async function updateEventBuffer(service: Service, delayMs: number = 8000): Promise<{ success: boolean; rateLimit?: RateLimitInfo }> {
  ...
  try {
    // Cal.com API buffer time configuration
    // Buffer time is specified in minutes
    const updateData: any = {
      beforeEventBuffer: BUFFER_TIME_MINUTES,
    };

    console.log(`\n⏱️  Updating buffer time for: ${service.name}`);
    ...
    if (response.status === 200 || response.status === 204) {
      console.log(`   ✅ Success! Buffer time updated for "${service.name}"`);
    ...
    if (error.response?.status === 429) {
      ...
      return updateEventBuffer(service, waitTime);
    }

    console.error(`   ❌ Failed to update buffer time for "${service.name}":`);
...
async function main() {
  console.log('🚀 Starting Cal.com buffer time update (ULTRA-SAFE MODE)...\n');
  console.log('⚠️  ONE API call at a time with status checking after each call\n');
  console.log(`⏱️  Buffer Time: ${BUFFER_TIME_MINUTES} minutes before each appointment\n`);
  ...
  for (let i = 0; i < servicesToUpdate.length; i++) {
    ...
    const result = await updateEventBuffer(service);
  ...
  console.log('\n✅ Buffer time update process complete!');
  console.log(`⏱️  All updated events now have a ${BUFFER_TIME_MINUTES}-minute buffer before appointments.`);
}
